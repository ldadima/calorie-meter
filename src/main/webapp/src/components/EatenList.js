...
export default class EatenList extends React.Component {
    ...
    setNormalCalorie() {
        axios.get("http://" + (process.env.NODE_ENV === "production" ? "10.170.0.2" : "localhost")   + ":8080/calorie-meter/user/calculateNorm?login=" + localStorage.getItem('login'))
    ...
    findAllFood(currentPage) {
        currentPage -= 1;
        axios.get("http://" + (process.env.NODE_ENV === "production" ? "10.170.0.2" : "localhost")   + ":8080/calorie-meter/user/userFoods?page=" + currentPage + "&size=" + this.state.foodsPerPage + "&login=" + localStorage.getItem('login'))
            .then(response => response.data)
            .then((data) => {
                this.setState({
    ...
    deleteFood = (foodId) => {
        axios.delete("http://" + (process.env.NODE_ENV === "production" ? "10.170.0.2" : "localhost")   + ":8080/calorie-meter/user/deleteFood/?login=" + localStorage.getItem('login') + "&food=" + foodId)
            .then(() => {
                this.setState({
                    "show": true,
    ...
    deleteAll = () => {
        axios.delete("http://" + (process.env.NODE_ENV === "production" ? "10.170.0.2" : "localhost")   + ":8080/calorie-meter/user/deleteAll/?login=" + localStorage.getItem('login'))
            .then(() => {
                this.setState({
                    "show": true,
                    "message": 'Удалено успешно'
